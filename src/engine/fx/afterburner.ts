...
function glowVertexMask (mesh: THREE.Mesh): Uint8Array | null {
  ...
  materials.forEach((material, slot) => {
    if (material && (/glow/i).test(material.name))
      glowSlots.add(slot)
  })
  if (!glowSlots.size)
    return null
  ...
  for (const group of groups) {
    if (!glowSlots.has(group.materialIndex ?? 0))
      continue
