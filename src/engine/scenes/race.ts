...
import { createApp, createSeededRng, defineModule, type App } from 'threejs-scene';
...
import { vehicleModule, type VehicleHandle } from '../modules/vehicle';
...
import { sunModule, type SunHandle } from '../modules/sun';
import { publishModule } from '../modules/publish';
import { attachBridge } from '../bridge';
import { initialRaceState, type RaceState } from '../state';
...
export async function mountRace(
  ...
  const vehicle: { current: VehicleHandle | null } = { current: null };
  const sun: { current: SunHandle | null } = { current: null };
  ...
  const isVehicleCollider = (handle: number) => {
    ...
    for (let i = 0; i < body.numColliders(); i++) {
      if (body.collider(i).handle === handle) return true;
    }
    return false;
  };
  ...
  const app = createApp<RaceState>(canvas, {
    ...
    render(frame) {
      ...
      if (composer) composer.render(frame.delta);
      else app.ctx.renderer.render(app.ctx.scene, rig.camera);
    },
  });
