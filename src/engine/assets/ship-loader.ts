...
import { SHIP_PRESETS, applyShipConfig, getFitTransform, type ShipConfig } from '@/lib/ship/materials';
import type { ShipId } from '@/lib/ship/registry';
import { createAfterburner, deriveNozzles, type Afterburner } from '../fx/afterburner';
...
export function preloadShip(shipId: ShipId): void {
  const preset = SHIP_PRESETS[shipId];
  if (preset.kind === 'fbx') void loadFbxTemplate(preset.path);
  else if (preset.kind === 'gltf') void loadGltfTemplate(preset.path);
}
