...
function drawFlightSight (overlay: HudPanel, frame: HudFrame): void {
  ...
  if (_velocity.lengthSq() > 4) {
    ...
    if (_pathPoint.visible) {
      ...
      drawFlightPathMarker(overlay, _pathPoint.x, _pathPoint.y, unit * 0.018, COLORS.green)
    }
  }

  const slip = slipFrom(frame.hullQuaternion, frame.telemetry.velocity)
  context.save()
  context.strokeStyle = Math.abs(slip) > 0.35 ? COLORS.amber : COLORS.blue
  ...
  overlayLabel(overlay, `${Math.round(pitch)}° PITCH · ${Math.round(roll)}° BANK · ${frame.telemetry.gLoad.toFixed(1)}G`, x, y + unit * 0.25, COLORS.blue)
}
...
function drawGunSight (overlay: HudPanel, battle: Extract<HudData, { mode: 'battle' }>['battle'], frame: HudFrame): void {
  ...
  const color               = locked ? COLORS.magenta : lock.phase === 'tracking' ? COLORS.amber : COLORS.white
  ...
  if (lock.phase !== 'idle') {
    ...
    context.lineWidth   = 3
    context.beginPath()
    context.arc(_aimPoint.x, _aimPoint.y, unit * 0.05, -Math.PI * 0.5, -Math.PI * 0.5 + TAU * THREE.MathUtils.clamp(lock.progress, 0, 1))
    context.stroke()
    if (locked) {
      const size          = unit * 0.062
      context.globalAlpha = 0.8
      context.strokeRect(_aimPoint.x - size, _aimPoint.y - size, size * 2, size * 2)
    }
    context.restore()
  }
  ...
  if (sight.impact) {
    ...
    if (_impactPoint.visible) {
      drawImpactMark(overlay, _impactPoint.x, _impactPoint.y, unit * 0.02, sight.onTarget ? COLORS.red : COLORS.cyan)
...
function drawKillFeed (
  ...
  const { context, canvas } = overlay
  context.save()
  context.font         = `500 ${Math.max(10, unit * 0.019)}px ${FONT}`
  ...
  battle.killFeed.slice(0, 3).forEach((entry, index) => {
    ...
    context.fillText(
      ...
      canvas.width - unit * 0.04,
      unit * 0.08 + index * unit * 0.035
    )
  })
  context.restore()
}
...
function overlayLabel (overlay: HudPanel, value: string, x: number, y: number, color: string): void {
  ...
  const unit                = Math.min(canvas.width, canvas.height)
  context.save()
  context.font         = `500 ${Math.max(10, unit * 0.021)}px ${FONT}`
