...
import type { Vec3 } from 'Δengine/battle/hitscan'
import type { BattlePlayer } from 'Δengine/battle/sim'
...
export class RewindBuffer {
   ...
   */
  record (tick: number, players: readonly BattlePlayer[]): void {
    ...
    for (let i = 0; i < players.length; i++) {
      const t                = players[i].chassis.translation()
   ...
   */
  poseSourceAt (tick: number): ((player: BattlePlayer) => Vec3) | null {
